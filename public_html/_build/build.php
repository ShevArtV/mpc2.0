...
class modExtraPackage
{
     ...
     */
    /**
     * Гарантирует, что services/vendor/composer/*-autoload не ссылается на
     * dev-зависимости, которые не попадают в transport-пакет. Предпочтительно
     * перегенерировать autoload через composer без dev; если composer недоступен
     * на сборочном сервере — детерминированно вычищаем dev-записи из files-autoload.
     *
     * @param string $servicesDir абсолютный путь к каталогу services/
     */
    protected function prepareVendorProdAutoload($servicesDir)
    {
        $composerDir = $servicesDir . '/vendor/composer';
        if (!is_dir($composerDir)) {
            return;
        }

        // 1) Честная перегенерация боевого autoload (чистит files + classmap + static).
        $regenerated = false;
        if (function_exists('exec') && is_file($servicesDir . '/composer.json')) {
            $out = [];
            $code = 1;
            @exec('cd ' . escapeshellarg($servicesDir)
                . ' && composer dump-autoload --no-dev --optimize --no-interaction 2>&1', $out, $code);
            if ($code === 0) {
                $regenerated = true;
                $this->modx->log(modX::LOG_LEVEL_INFO, '[build] vendor autoload перегенерирован без dev (composer)');
                // Вернуть dev-autoload после сборки, чтобы не ломать локальные тесты.
                register_shutdown_function(static function () use ($servicesDir) {
                    @exec('cd ' . escapeshellarg($servicesDir)
        ...
        // 2) Fallback/страховка: если composer недоступен — вручную опустошаем
        //    files-autoload (единственное, что грузится eagerly; classmap/psr-4
        //    на dev-классы ленивы и в проде безвредны).
        if (!$regenerated) {
            $filesPhp = $composerDir . '/autoload_files.php';
            if (is_file($filesPhp)) {
                file_put_contents($filesPhp,
                    "<?php\n\n// autoload_files.php @generated by Composer\n\n"
                    . "\$vendorDir = dirname(__DIR__);\n\$baseDir = dirname(\$vendorDir);\n\n"
                    . "return array();\n");
            }
            $staticPhp = $composerDir . '/autoload_static.php';
            if (is_file($staticPhp)) {
                $src = file_get_contents($staticPhp);
                $src = preg_replace(
                    '/(public\s+static\s+\$files\s*=\s*array\s*\()[^;]*?(\);)/s',
                    "$1\n    $2",
                    $src, 1);
                file_put_contents($staticPhp, $src);
            }
            $this->modx->log(modX::LOG_LEVEL_INFO, '[build] vendor files-autoload вычищен от dev (fallback)');
        }
    }
    ...
    public function process()
    {
        ...
        // Привести services/vendor к боевому виду ДО упаковки: composer-генератор
        // прописывает в files-autoload dev-зависимости (myclabs/deep-copy,
        // phpunit/.../Functions.php), а $vendorKeep ниже физически выкидывает их из
        // пакета. files-autoload грузится eagerly при vendor/autoload.php → require
        // несуществующего файла → Fatal, кладёт сайт при установке (инцидент 2026-06-16).
        $this->prepareVendorProdAutoload(rtrim($this->config['core'], '/\\') . '/services');
