...
class Render extends Base
{
    ...
    /**
     * Нормализует значения внутри блока параметров `[ … ]`: для каждого значения
     * после '=>' квотируется только ПРОСТОЕ «голое слово» (резолв eager-плейс-
     * холдера, например about-us). Всё остальное — выражения ($…, (…), {…}),
     * массивы, числа/булево/null и уже квотированное — остаётся как есть.
     * Вложенные массивы проходятся рекурсивно (их значения тоже квотируются),
     * но сам массив не квотируется.
     */
    private function quoteBareValues(string $s): string
    {
        ...
        while ($i < $len) {
            ...
            if ($ch === '=' && $i + 1 < $len && $s[$i + 1] === '>') {
                ...
                $end   = $this->readValueEnd($s, $i);
                $raw   = substr($s, $i, $end - $i);
                $core  = rtrim($raw);
                $trail = substr($raw, strlen($core));
                $out  .= $this->normalizeValue($core) . $trail;
                $i = $end;
                continue;
            }
    ...
    /**
     * Решает, надо ли квотировать значение параметра. Квотируем ТОЛЬКО простое
     * голое слово (буквы/цифры/_/./-/слеш) — это резолв eager-плейсхолдера.
     * Выражения (($…, (…), {…}, @…), уже квотированное, числа/булево/null —
     * не трогаем. Массив ([…]) — рекурсивно нормализуем содержимое.
     */
    ...
    /**
     * Возвращает позицию конца значения (на разделителе ',' или закрывающей ']'
     * текущего уровня), корректно пропуская вложенные () [] {} и строковые
     * литералы.
     */
    private function readValueEnd(string $s, int $i): int
    {
        $len    = strlen($s);
        $square = 0;
        $round  = 0;
