...
$_lang['setting_mpc_theme'] = 'Тема оформления';
$_lang['setting_mpc_theme_desc'] = 'Активная тема на весь сайт. Тема — подпапка-зеркало внутри пути к секциям; для секции берётся sections/<тема-папка>/<тема>/<секция>.tpl, если файла нет — базовая вёрстка. Контент не затрагивается. Пусто → базовая вёрстка.';
$_lang['setting_mpc_theme_templates'] = 'Темы по шаблонам';
$_lang['setting_mpc_theme_templates_desc'] = 'Переопределение темы для отдельных шаблонов, JSON вида {"5":"dark","12":"summer"}. Приоритетнее настройки «Тема оформления». Пусто → действует общая тема.';
$_lang['setting_mpc_themes_subdir'] = 'Папка тем';
$_lang['setting_mpc_themes_subdir_desc'] = 'Подпапка внутри пути к секциям, где лежат темы. Значение по умолчанию _themes/.';
$_lang['setting_mpc_path_to_src'] = 'Путь к размеченной верстке';
$_lang['setting_mpc_path_to_src_desc'] = 'значение по умолчанию templates/. Задаётся относительно pdotools_elements_path.';
...
$_lang['setting_mpc_lang_cookie_name'] = 'Имя cookie выбранного языка';
$_lang['setting_mpc_lang_cookie_name_desc'] = 'Имя cookie, в котором хранится выбранный язык (по умолчанию mpc_lang). Одинаковое имя на домене и поддоменах → язык общий; задайте контекстной настройкой разное имя, чтобы языки на поддоменах были независимы.';
$_lang['setting_mpc_lang_cookie_domain'] = 'Домен cookie языка';
$_lang['setting_mpc_lang_cookie_domain_desc'] = 'Домен, на котором ставится cookie языка. Пусто — текущий хост (http_host). Укажите базовый домен с ведущей точкой (например .site.ru), чтобы cookie была доступна на всех поддоменах.';
$_lang['setting_mpc_set_language_on_request'] = 'Выставлять язык на запросе';
$_lang['setting_mpc_set_language_on_request_desc'] = 'Вкл (по умолчанию) — пакет на событии OnHandleRequest сам выставляет язык (cultureKey + cookie). Выкл — пакет язык не трогает, проект делает это сам (например, после switchContext). Для тонкой настройки без полного выключения (изменить набор языков/язык по умолчанию/имя cookie или пропустить) используйте событие mpcOnBeforeSetLanguageSettings.';
...
$_lang['setting_mpc_contact_lexicon_fields_desc'] = 'под-поля контакта, которые лексиконятся, картой тип→поля. Записи через запятую: «поле» — для всех типов, «тип:поле» — только для контакта этого типа (phone/email/social/address/map/worktime/requisite/messenger). Поля: caption/value/fvalue/attributes. Пример: «caption, address:value, address:fvalue» — caption у всех + значение только у адреса. По умолчанию caption. Точечно переопределяется в вёрстке атрибутом data-mpc-translate на блоке контакта.';
