...
$_lang['setting_mpc_theme'] = 'Design theme';
$_lang['setting_mpc_theme_desc'] = 'Active theme for the whole site. A theme is a mirror subfolder inside the sections path; a section is loaded from sections/<themes-subdir>/<theme>/<section>.tpl, falling back to the base markup when the file is missing. Content is not affected. Empty → base markup.';
$_lang['setting_mpc_theme_templates'] = 'Themes per template';
$_lang['setting_mpc_theme_templates_desc'] = 'Per-template theme override, JSON like {"5":"dark","12":"summer"}. Takes precedence over the «Design theme» setting. Empty → the global theme applies.';
$_lang['setting_mpc_themes_subdir'] = 'Themes subfolder';
$_lang['setting_mpc_themes_subdir_desc'] = 'Subfolder inside the sections path where themes live. Default value _themes/.';
...
$_lang['setting_mpc_set_language_on_request'] = 'Set language on request';
$_lang['setting_mpc_set_language_on_request_desc'] = 'On (default) — the package sets the language (cultureKey + cookie) on the OnHandleRequest event. Off — the package does not touch the language; the project handles it itself (e.g. after switchContext). For fine control without fully disabling (change the language set / default language / cookie name, or skip) use the mpcOnBeforeSetLanguageSettings event.';
...
$_lang['setting_mpc_contact_lexicon_fields_desc'] = 'contact sub-fields that get lexiconized, as a type→fields map. Comma-separated entries: "field" — for all contact types, "type:field" — only for a contact of that type (phone/email/social/address/map/worktime/requisite/messenger). Fields: caption/value/fvalue/attributes. Example: "caption, address:value, address:fvalue" — caption for all + value only for address. Default caption. Can be overridden per block in the template via the data-mpc-translate attribute.';
